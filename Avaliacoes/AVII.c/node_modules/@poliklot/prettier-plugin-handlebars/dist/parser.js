...
const template_format_core_3 = require("template-format-core");
const template_format_core_4 = require("template-format-core");
const tokens_1 = require("./dialects/handlebars/tokens");
const templateDialect = tokens_1.handlebarsDialect;
function parse(text) {
...
function parseMustacheToken(text, position) {
...
function isDynamicTagStart(text, position) {
    return templateDialect.isDynamicElementStart(text, position);
}
function consumeRawBlock(text, position) {
    ...
    if (endTag && template_format_core_1.rawTextElements.has(endTag.toLowerCase())) {
        ...
        if (rawContent.length > 0) {
            nodes.push((0, template_format_core_2.withRange)({
                ...
                value: rawContent,
                verbatim: true,
                preserveWhitespace: template_format_core_1.whitespaceSensitiveRawTextElements.has(endTag.toLowerCase()),
    ...
    while (pos < text.length) {
        ...
        if (startsTemplateTag(text, pos)) {
            ...
            if (shouldPreserveMustacheVerbatim(token) && !(endBlock && token.kind === 'else')) {
                const preserveEnd = token.kind === 'blockStart' ? consumeUnsupportedBlock(text, pos, token) : token.end;
            ...
            if (token.kind === 'blockEnd') {
                ...
                nodes.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: text.slice(pos, token.end) }, rangeOffset + pos, rangeOffset + token.end));
                pos = token.end;
                continue;
            }
            ...
            if (token.specialForm === 'decorator') {
                nodes.push(createDecorator(token.content.slice(1).trim(), token.trimOpen, token.trimClose, rangeOffset + pos, rangeOffset + token.end));
                pos = token.end;
                continue;
            }
            if (token.kind === 'comment') {
        ...
        if (text[pos] === '<') {
            if (text.startsWith('<!', pos) && !text.startsWith('<!--', pos)) {
                ...
                nodes.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: text.slice(pos, end), verbatim: true }, rangeOffset + pos, rangeOffset + end));
                pos = end;
                continue;
            }
            if (!isTagStart(text, pos)) {
                const nextMarkup = findNextMarkup(text, pos + 1);
                nodes.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: text.slice(pos, nextMarkup) }, rangeOffset + pos, rangeOffset + nextMarkup));
                pos = nextMarkup;
                continue;
            }
            ...
            if (tagResult.kind === 'close') {
                ...
                nodes.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: text.slice(pos, tagResult.end), verbatim: true }, rangeOffset + pos, rangeOffset + tagResult.end));
                pos = tagResult.end;
                continue;
            }
            ...
            const { nodes: children, position: newPos, endReason: childEndReason, } = parseChildren(text, tagResult.end, tagResult.tag, null, rangeOffset);
        ...
        if (trimmed.length > 0) {
            ...
            nodes.push((0, template_format_core_2.withRange)(node, rangeOffset + pos, rangeOffset + nextMarkup));
        }
        else {
            const newlineCount = (rawValue.match(/\n/g) || []).length;
            const blankLines = Math.max(newlineCount - 1, 0);
            if (blankLines > 0) {
                nodes.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: '', blankLines }, rangeOffset + pos, rangeOffset + nextMarkup));
            }
        }
        pos = nextMarkup;
    }
...
function hasMatchingBlockEnd(text, token, start) {
    if (!token.name) {
        return false;
    }
    let depth = 0;
...
function parseBlock(text, token, rangeOffset = 0) {
    ...
    const { nodes: program, position: afterProgram, endReason, endToken } = parseChildren(text, token.end, null, openInfo.path, rangeOffset);
    const buildProgram = (nodes, start, end) => (0, template_format_core_2.withRange)({ type: 'Program', body: trimEdgeWhitespace(nodes) }, rangeOffset + start, rangeOffset + end);
    const programBody = buildProgram(program, token.end, afterProgram);
    let inverseBody = (0, template_format_core_2.withRange)({ type: 'Program', body: [] }, rangeOffset + afterProgram, rangeOffset + afterProgram);
    const inverseChain = [];
    ...
    if (endReason === 'else' && endToken) {
        ...
        while (currentElseToken?.specialForm === 'elseIf') {
            ...
            const { nodes: branchNodes, position: afterBranch, endReason: branchEndReason, endToken: branchEndToken, } = parseChildren(text, currentPosition, null, openInfo.path, rangeOffset);
        ...
        if (currentElseToken) {
            ...
            const { nodes: inverseNodes, position: afterInverse, endReason: inverseEndReason, endToken: inverseEndToken, } = parseChildren(text, currentPosition, null, openInfo.path, rangeOffset);
...
function getBlockExpression(token) {
    return templateDialect.getBlockExpression(token);
}
function getBlockPrefix(token) {
    return templateDialect.getBlockPrefix(token);
}
function shouldPreserveUnclosedBlockRemainder(token) {
    return templateDialect.shouldPreserveUnclosedBlockRemainder(token);
}
function hasMatchingTagEnd(text, tag, start, limit = -1) {
...
function isWhitespaceOnlyText(node) {
    return node.type === 'TextNode' && node.value === '';
}
...
function createUnmatchedNode(text, start, end) {
    return (0, template_format_core_2.withRange)({ type: 'UnmatchedNode', raw: text.slice(start, end) }, start, end);
}
function parseTag(text, position) {
...
function parseAttribute(text, position) {
    let pos = position;
    skipWhitespace(text, () => pos++, () => pos);
    const attrStart = pos;
    ...
    if (text[pos] === '"' || text[pos] === "'") {
        ...
        while (pos < text.length && text[pos] !== '>') {
            ...
            if (template_format_core_4.whitespace.test(text[pos])) {
                break;
            }
            pos += 1;
        }
...
function shouldPreserveRawAttribute(name, rawValue) {
...
function parseDynamicAttribute(text, position) {
    ...
    const nameEnd = pos;
    let afterName = pos;
    while (afterName < text.length && template_format_core_4.whitespace.test(text[afterName])) {
        afterName += 1;
    }
    if (text[afterName] !== '=') {
        return {
    ...
    pos = afterName + 1;
    while (pos < text.length && template_format_core_4.whitespace.test(text[pos])) {
        pos += 1;
    }
    if (text[pos] === '"' || text[pos] === "'") {
        const quote = text[pos];
        ...
        while (pos < text.length && !template_format_core_4.whitespace.test(text[pos]) && text[pos] !== '>') {
            if (isSelfClosingSlash(text, pos)) {
                break;
            }
            if (startsTemplateTag(text, pos)) {
...
function createAttribute(name, rawValue, valueStart) {
    ...
    return {
        type: 'Attribute',
        name,
        value: (0, template_format_core_2.withOptionalRange)({
            type: 'AttributeValue',
            parts,
        }, valueStart, typeof valueStart === 'number' ? valueStart + rawValue.length : undefined),
    };
}
...
function stringifyMustacheForAttribute(node) {
    const pieces = [];
    if (node.path) {
...
function parseAttributeValueParts(value, rangeOffset = 0) {
    ...
    while (pos < value.length) {
        if (startsTemplateTag(value, pos)) {
            ...
            if (token.specialForm === 'decorator') {
                parts.push(createDecorator(token.content.slice(1).trim(), token.trimOpen, token.trimClose, rangeOffset + pos, rangeOffset + token.end));
                pos = token.end;
                continue;
            }
            // обычный mustache
            if (token.kind === 'mustache') {
                parts.push(createMustache(token.content, token.triple, token.trimOpen, token.trimClose, rangeOffset + pos, rangeOffset + token.end));
                pos = token.end;
                continue;
            }
            // блок {{#if ...}} ... {{/if}}
            if (token.kind === 'blockStart') {
                if (!hasMatchingBlockEnd(value, token, pos)) {
                    ...
                    parts.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: value.slice(pos, token.end) }, rangeOffset + pos, rangeOffset + token.end));
                    pos = token.end;
                    continue;
                }
        ...
        if (rawText.length > 0) {
            parts.push((0, template_format_core_2.withRange)({ type: 'TextNode', value: rawText }, rangeOffset + pos, rangeOffset + end));
        }
        pos = end;
    }
    return parts;
...
function skipWhitespace(text, advance, getPos) {
    while (getPos() < text.length && template_format_core_4.whitespace.test(text[getPos()])) {
        advance();
    }
}
...
function shouldPreserveMustacheVerbatim(token) {
    return templateDialect.shouldPreserveTokenVerbatim(token);
}
function consumeUnsupportedBlock(text, position, openToken) {
    if (!openToken.name) {
...
function parseExpression(content) {
    const expression = (0, template_format_core_3.parseTemplateExpression)(content);
    return {
...
function createMustache(content, triple, trimOpen = false, trimClose = false, start, end) {
    const expression = (0, template_format_core_3.parseTemplateExpression)(content);
    const node = {
        type: 'MustacheStatement',
        triple,
...
function createPartial(content, trimOpen = false, trimClose = false, start, end) {
    const expression = (0, template_format_core_3.parseTemplateExpression)(content);
    const node = {
...
function createDecorator(content, trimOpen = false, trimClose = false, start, end) {
    const expression = (0, template_format_core_3.parseTemplateExpression)(content);
    const node = {
...
function createComment(content, start, end) {
