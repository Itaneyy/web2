...
function findHandlebarsClose(text, position, closeDelimiter) {
    ...
    for (let index = position; index < text.length; index += 1) {
        ...
        if ((char === '"' || char === "'" || char === '`') && (0, template_format_core_1.isTemplateExpressionQuoteStart)(text, index, position)) {
            quote = char;
            continue;
        }
...
function getHandlebarsTagDelimiters(triple) {
...
function getHandlebarsPartialPrefix() {
    return '> ';
}
function getHandlebarsDecoratorPrefix() {
    return '*';
}
...
function getHandlebarsLineCommentTag(value) {
    if (value.startsWith('<')) {
        return `{{!${value}}}`;
    }
    return `{{! ${value}}}`;
}
function getHandlebarsBlockCommentTag(value) {
    return `{{!-- ${value} --}}`;
}
...
function shouldPreserveUnclosedHandlebarsBlockRemainder(token) {
