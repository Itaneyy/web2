...
exports.altera_post = async function (req, res) {
  ...
  var desc = req.body.desc;
  var prioridade = req.body.prioridade;
  var executada = req.body.executada;

  // atualiza a nota com a chave
  await banco.atualiza(chave, titulo, desc, prioridade, executada);
  // redireciona para a página de consulta da nota
  res.redirect("/atividade/consulta/" + chave);
};
...
exports.MarcaComoFeita = async (req, res) => {
  var chave = req.params.chave_nota;

  await banco.toggle(chave);
