...
const lista_atividades = []; //será um vetor de objetos do tipo Nota
var id = 0;

class NotaMemoria {
  async atualiza(id, titulo, desc, prioridade, executada) {
    lista_atividades[id] = new Atividade(titulo, desc, prioridade, executada);
  ...
  async consulta(id) {
    ...
    else throw new Error(`Nota com o id ${id} não existe`);
  }

  async deleta(id) {
    if (lista_atividades[id]) {
      delete lista_atividades[id];
    } else throw new Error(`Nota com a id ${id} não existe`);
  }
  ...
  async toggle(id) {
    lista_atividades[id].executada = !lista_atividades[id].executada;
  }
}

// CORREÇÃO: Aqui deve ser NEW NOTAMEMORIA e não Atividade
var banco = new NotaMemoria();
